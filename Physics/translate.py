...
def translate_velocity(spline_diff, b,N_discretization):
    ...
    for i in range(N_discretization-1):
        v[i] = np.sqrt((spline_derivative[0][i]*np.sqrt((b[i]+b[i+1])/2))**2+(spline_derivative[1][i]*np.sqrt((b[i]+b[i+1])/2))**2)
    return  v
...
def translate_acceleration(spline_diff, spline_second_diff, b, a,N_discretization):
    ...
    for i in range(N_discretization-1):
        a[i] = np.sqrt((spline_derivative[0][i]*a[i])**2+(spline_derivative[1][i]*a[i])**2)+ np.sqrt((spline_second_derivative[0][i]*(b[i]+b[i+1])/2)**2+(spline_second_derivative[1][i]*(b[i]+b[i+1])/2)**2)
    return  a
