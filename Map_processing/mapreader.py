...
def get_outline(doc1, doc2):
    ...
    for i in range(len(coord_matrix2[0])-1):
        coord_matrix2[0,i]=(my_map2.points[0][i][0]-my_map1.points[0][0][0])*np.pi*a*np.cos(my_map1.points[0][0][0]*np.pi/180)/(180*np.sqrt(1-e2*np.sin(my_map1.points[0][0][0]*np.pi/180)))
        coord_matrix2[1,i]=(my_map2.points[0][i][1]-my_map1.points[0][0][1])*np.pi*a*(1-e2)/(180*pow(1-e2*pow(np.sin(my_map1.points[0][0][1]*np.pi/180),2),3/2))
