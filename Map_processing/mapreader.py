...
def get_outline(doc1, doc2):
    ...
    my_map1 = gpd.read_file(doc1, driver='KML') #read kml of each side of the track
    my_map2 = gpd.read_file(doc2, driver='KML')

    my_map1['points'] = my_map1.apply(lambda x: [y for y in x['geometry'].coords], axis=1) #extract the coordinates from the kml 
    ...
    #earth eliptical constants
    a=6378137 #radii
    b=6356752.3142
    e2=(a**2-b**2)/a**2 #excentricity
    
    #translate geografical coordinates into meters and set the origin as the first point
