...
def create_objective(xsi,A_t,F_1t,F_2t,T0,E0,n_discretization,n_wheels,expansion_factor):
...
def create_Hessian_objective(xsi,T0,n_discretization,n_wheels,expansion_factor):
...
def create_friction_circle(mu,mass,F_1t,F_2t,n_discretization,n_wheels,expansion_factor):
    ...
    def friction_circle(t):
        ...
        for i in range(n_discretization-1):
            ...
            for j in range(n_wheels):
                remainder[i+j*(n_discretization-1)] =-(mu*mass*9.81/n_wheels)**2+\
...
def create_friction_circle_jacobian(n_discretization,F_1t,F_2t,n_wheels,expansion_factor):
    ...
    def friction_circle_jac(t):
        ...
        for i in range(n_discretization-1):
            for j in range(n_wheels):
                B1[i+j*(n_discretization-1),i] = 2*np.dot(F_2t[i][2*j:2*j+2],(x[i+1]
                    *F_1t[i][2*j:2*j+2]+x[i]*F_2t[i][2*j:2*j+2]))
                B1[i+j*(n_discretization-1),i+1] = 2*np.dot(F_1t[i][2*j:2*j+2],(x[i+1]
                    *F_1t[i][2*j:2*j+2]+x[i]*F_2t[i][2*j:2*j+2]))
...
#creates friction circle constraints gradient
def create_friction_circle_Hessian(n_discretization,F_1t,F_2t,n_wheels,expansion_factor):
    ...
    def friction_circle_Hessian(x,v):
        ...
        for i in range(n_discretization-1):
            for j in range(n_wheels):
                ...
                Hessian_i = sparse.lil_matrix((n_discretization,n_discretization))
                Hessian_i[i,i]=2*np.dot(F_2t[i][2*j:2*j+2],F_2t[i][2*j:2*j+2])
                Hessian_i[i+1,i]=2*np.dot(F_1t[i][2*j:2*j+2],F_2t[i][2*j:2*j+2])
                Hessian_i[i+1,i+1]=2*np.dot(F_1t[i][2*j:2*j+2],F_1t[i][2*j:2*j+2])
                Hessian_i[i,i+1]=2*np.dot(F_2t[i][2*j:2*j+2],F_1t[i][2*j:2*j+2])
                B+=v[i]*Hessian_i
                B+=v[i]*Hessian_i
...
#Output result
def optimization_SQP_b_3(R_t,M_t,C_t,A_t,n_discretization,xsi,n_wheels,display):
    ...
    obj = create_objective(xsi,A_t,F_1t,F_2t,T0,E0,n_discretization,n_wheels,expansion_factor)
    obj_grad = create_gradient_objective(xsi,A_t,F_1t,F_2t,T0,E0,n_discretization,n_wheels,expansion_factor)
    obj_hess = create_Hessian_objective(xsi,T0,n_discretization,n_wheels,expansion_factor)
    ...
    friction_circle, lb_fc, ub_fc = create_friction_circle(mu,mass,F_1t,F_2t,n_discretization,n_wheels,expansion_factor)
    Grad_fc = create_friction_circle_jacobian(n_discretization,F_1t,F_2t,n_wheels,expansion_factor)
    Hessian_fc = create_friction_circle_Hessian(n_discretization,F_1t,F_2t,n_wheels,expansion_factor)
