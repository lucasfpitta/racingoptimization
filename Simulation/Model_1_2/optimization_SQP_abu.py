...
def create_gradient_objective(xsi,A_t,T0,E0,n_discretization):
    ...
    def Grad(x):
        ...
        for i in range(n_discretization-1):
            ...
            f[b+i+1]=2*xsi/T0*(-1/(2*x[b+i+1]**0.5*(x[b+i+1]**0.5+x[b+1-i]**0.5)**2))
        return f
    return Grad
    ...
    def Hessian(x):
        ...
        for i in range(n_discretization-1):
            H[b+i][b+i]=H[b+i][b+i]+2*xsi/T0*(x[b+i+1]**0.5+3*x[b+i]**0.5)/(4*x[b+i]**1.5*(x[b+i]**0.5\
                +x[b+i+1]**0.5)**3)
            H[b+i+1][b+i+1]= 2*xsi/T0*(x[b+i-1]**0.5+3*x[b+i]**0.5)/(4*x[b+i]**1.5*(x[b+i]**0.5\
                +x[b+i-1]**0.5)**3)
            H[b+i][b+i+1] = 2*xsi/T0/(2*(x[b+i]*x[b+i+1])**0.5*(x[b+i]**0.5+x[b+i+1]**0.5)**3)
            H[b+i+1][b+i] = 2*xsi/T0/(2*(x[b+i]*x[b+i-1])**0.5*(x[b+i]**0.5+x[b+i-1]**0.5)**3)
        return H
    return Hessian
