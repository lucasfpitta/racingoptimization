...
def create_objective_vector(xsi, A_t, F_1t, F_2t,T0,E0,n_discretization):
    ...
    for i in range(n_discretization-2):
        ...
        f[d+i]=2*xsi/T0
    f[n_discretization-1]=(1-xsi)*(F_1t[n_discretization-2].T@A_t[n_discretization-2])/E0
    f[-1]=f[d]=2*xsi/T0
    return f
...
def create_b_c_cones(x,n_discretization):
    ...
    for i in range(n_discretization):
        ...
        c_vec=np.zeros(2*n_discretization+n_discretization-1)
        c_vec[i] = 1
        #build the cone matrix A_matrix, which is 2 for c_k in the first line, 1 for b_k in the
...
def create_c_d_cones(x,n_discretization):
    ...
    for i in range(n_discretization-1):
        #build the cone vector c_vec, which is 1 for d_k, c_{k+1}, c_k and 0 otherwise
        c_vec=np.zeros(2*n_discretization+n_discretization-1)
        c_vec[d+i]=1
        c_vec[c+i]=1
        c_vec[c+1+i]=1
        #build the cone matrix A_matrix, which is 1 for d_k, -1 for c_{k+1}, -1 for c_k in the
        #second line, and 0 otherwise
...
def optimization_b_SOCP(R_t,M_t,C_t,A_t,n_discretization,xsi,display):
    ...
    soc_constraints.extend(create_friction_circle_cones(x,F_1t, F_2t,n_discretization,mass,mu))
    
    #set the SOCP problem
    prob = cp.Problem(cp.Minimize(f.T@x),soc_constraints)#,soc_constraints+[F @ x == g]
    prob.solve()
