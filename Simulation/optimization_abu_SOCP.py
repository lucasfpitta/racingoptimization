...
def create_equality_constraint_matrix(R_t,M_t,C_t,n_discretization):
    ...
    F = np.zeros((3*(n_discretization-1),2*n_discretization+4*(n_discretization-1)))
    
    #iterate over each section to have the dynamics constraint on the two cartesian coordinates 
...
def create_friction_circle_cones(x,n_discretization,m,mu):
    ...
    for i in range(n_discretization-1):
        #build the cone matrix A_matrix, which is 1 for u_1k on the first line and for u_2k on the second line, 
        # and 0 otherwise
        A_matrix = np.zeros((2,2*n_discretization+4*(n_discretization-1)))
        A_matrix[0][u1+i]=1
        A_matrix[1][u2+i]=1
